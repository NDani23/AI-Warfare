...
public class TankManager : VehicleManager, ITargetable
{
    ...
    public void FixedUpdate()
    {
        ...
        var targets = _envController.VehicleList;

        foreach(var target in targets)
        {
            if(target.GetComponent<VehicleManager>().Team == this._team) continue;
            if(target.GetComponent<VehicleManager>().Health <= 0) continue;
            if(target.GetComponent<VehicleManager>().Detected) continue;
    ...
    public void Update()
    {
        ...
        if (activeCommand == CommandType.EliminateTarget)
        {
            if (_commandMarker.FollowTarget == null || 
                _commandMarker.FollowTarget.GetComponent<VehicleManager>().Health <= 0 ||
                _commandMarker.FollowTarget.GetComponent<VehicleManager>().Detected == false)
            {
                _envController.AssignDefaultCommand(this);
            }
        }
    ...
    protected override void extendResetVehicle()
    {
        // ClearCommand();
        // _envController.AssignDefaultCommand(this);
    }
